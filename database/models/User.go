...
func (r *userR) GetUserIdEducations() EducationSlice {
	if r == nil {
		return nil
	}
...
var (
	userAllColumns            = []string{"ID", "Name", "Address"}
	userColumnsWithoutDefault = []string{"Name", "Address"}
...
// UserIdEducations retrieves all the Education's Educations with an executor via UserId column.
func (o *User) UserIdEducations(mods ...qm.QueryMod) educationQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`Education`.`UserId`=?", o.ID),
	)
...
func (userL) LoadUserIdEducations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`Education`),
		qm.WhereIn(`Education.UserId in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on Education")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for Education")
	}
	...
	if singular {
		object.R.UserIdEducations = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserId {
				local.R.UserIdEducations = append(local.R.UserIdEducations, foreign)
				if foreign.R == nil {
					foreign.R = &educationR{}
				}
...
func (o *User) AddUserIdEducationsG(ctx context.Context, insert bool, related ...*Education) error {
	return o.AddUserIdEducations(ctx, boil.GetContextDB(), insert, related...)
}

// AddUserIdEducations adds the given related objects to the existing relationships
// of the User, optionally inserting them as new records.
// Appends related to o.R.UserIdEducations.
// Sets related.R.UserIdUser appropriately.
func (o *User) AddUserIdEducations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Education) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			UserIdEducations: related,
		}
	} else {
		o.R.UserIdEducations = append(o.R.UserIdEducations, related...)
	}
